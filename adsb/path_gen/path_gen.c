...
void help(char pname[])                //Display simple instructions.
{
  ...
  printf("  The 13 fields are as follows:\n");
  ...
  Pause("    Hit ENTER to continue to information on noise and perturbation: \n");
  printf("  The following 6 values represent noise to generated values,\n");
  printf("  just before they are written to file and should be small floats.\n");
  printf("  These will represent noise added to the actual data, not a\n");
  printf("  change in the data itself (see the next section for that).\n");
  printf("  Thus, these will not cause a change in the generated path.\n");
  printf("    5) noise_alt float representing random noise added to the\n");
  printf("      altitude. This value will be *50 then + or - to the alt.\n");
  printf("    6) noise_lat  + or - 0.0 to this amount added as noise\n");
  printf("    7) noise_lon  + or - 0.0 to this amount added as noise\n");
  printf("    8) noise_vel float value adjustment as noise to velocity but\n");
  printf("       *100 then + or - adjustment to the actual velocity.\n");
  printf("    9) noise_dir float adjustment as noise to direction (in rads)\n");
  printf("      divided by 2 then + or - adjustment to actual direction.\n");
  printf("   10) noise_vr *100 then +/- random amount to vertical rate.\n");
  printf("  The following 3 values represent actual perturbations to the \n");
  printf("  generated values and should be small floats, such as 0.05.\n");
  printf("   11) var_alt float representing the random amount the\n");
  printf("      altitude will be adjusted. This value will be *50 then\n");
  printf("      + or - to the actual altitude.\n");
  printf("   12) var_vel float value adjustment for velocity but *100 then\n");
  printf("      + or - adjustment to actual velocity.\n");
  printf("   13) var_dir float adjustment for direction (in radians),\n");
  printf("      divided by 2 then + or - adjustment to actual direction.\n");
  printf("  Either space or comma can be used as a separater.    Example:\n");
  printf("0  1  0  0  0.05  0.05  0.05  0.05  0.05  0.05  0.0  0.0  0.0");
...
int Initial_wp_file(void)
{
  ...
  if(!(strlen(line)>3 && (int)line[strlen(line)-2]>57))  //processing.
  {
    ...
    noise_alt=AtoF(line, &i);          //Get the three noise factors.
    noise_lat=AtoF(line, &i);
    noise_lon=AtoF(line, &i);
    noise_vel=AtoF(line, &i);
    noise_dir=AtoF(line, &i);
    noise_vr=AtoF(line, &i);
    var_alt=AtoF(line, &i);            //Get the three perturbation factors.
    var_vel=AtoF(line, &i);
    var_dir=AtoF(line, &i);
  ...
  if(noise_alt<0.0 || noise_alt>0.5)   //Keep noise within reason.
    noise_alt=0.0;
  if(noise_lat<0.0 || noise_lat>0.5)   //Keep noise within reason.
    noise_lat=0.0;
  if(noise_lon<0.0 || noise_lon>0.5)   //Keep noise within reason.
    noise_lon=0.0;
  if(noise_vel<0.0 || noise_vel>0.5)   //Keep noise within reason.
    noise_vel=0.0;
  if(noise_dir<0.0 || noise_dir>0.5)   //Keep noise within reason.
    noise_dir=0.0;
  if(noise_vr<0.0 || noise_vr>0.5)     //Keep noise within reason.
    noise_vr=0.0;
...
//       sec_mission_start,var_time,noise_alt,noise_lat,noise_lon,
//       noise_vel,noise_dir, noise_vr);
//printf("  Perturbation factors: %f, %f, %f\n",var_alt,var_vel,var_dir);
//Pause2(0);
...
void Write_gps_msg(FILE *msgf, int pause_time, int msg_cnt)
{
  ...
  float tmp1;                          //Used for noise level determination.
  double tmp_dbl;
  ...
  //Create a noise value between +/- noise_lat.
  tmp1=noise_lat*((float)(rand()%200)*0.01)-noise_lat;
  //Add noise offset then convert to GPS message units.
  tmp32=(uint32_t)(0.5+(cur_data.lat_deg+tmp1)/cir_deg32);
  ...
  //Create a noise value between +/- noise_lon.
  tmp1=noise_lon*((float)(rand()%200)*0.01)-noise_lon;
  //Add noise offset then convert to GPS message units.
  tmp32=(uint32_t)(0.5+(cur_data.lon_deg+tmp1)/cir_deg32);
  ...
  tmp1=(rand()%100)*noise_alt-noise_alt*50.0;  //Determine noise factor.
  tmp_dbl=(cur_data.alt+tmp1)*0.25+0.5;
  ...
  //bit two's compliment word, and add in noise component.
  tmp1=((rand()%100)*noise_vel*2.0-100.0*noise_vel);
  tmp_dbl=(cur_data.vEast+tmp1)/vel_div+0.5;  //Convert to GPS msg units.
  ...
  tmp1=((rand()%100)*noise_vel*2.0-100.0*noise_vel);
  tmp_dbl=(cur_data.vNorth+tmp1)/vel_div+0.5;  //Convert to GPS msg units.
  ...
  tmp1=((rand()%100)*noise_vr*2.0-100.0*noise_vr);
  tmp_dbl=(cur_data.vUp+tmp1)/vel_div+0.5;  //Convert to GPS msg units.
...
{
  float alt, vel, dir, vr;  //Used for noise determination.
  double lat, lon;
  CD cd=cur_data;

  alt=cd.alt+(rand()%100)*noise_alt-noise_alt*50.0;
  lat=cd.lat_dir*cd.lat_deg +noise_lat*((float)(rand()%200)*0.01)-noise_lat;
  lon=cd.lon_dir*cd.lon_deg +noise_lon*((float)(rand()%200)*0.01)-noise_lon;
  vel=cd.gps_vel +((rand()%100)*noise_vel*2.0-100.0*noise_vel);
  dir=cd.gps_dir +((rand()%100)*noise_dir/100.0-noise_dir*0.5);
  vr =cd.vUp +((rand()%100)*noise_vr*2.0-100.0*noise_vr);
